...
cifx_logger = text_logger.get_logger("CIFX70E_DP")
cifx_logger.debug("starting to use CIFX70E_DP module")
...
def WIC_RunCifXConsoleTest( szBoard, szFirmwareFile, szConfigFile, ulTimerResolution, ulIOTimeout):
    ...
    if CIFX_NO_ERROR != wic_dll.xDriverOpen(ctypes.byref(hDriver)):
        print("xDriver is opened correctly, continuing to operate functions")
        return
    else:
        print("xDriver failed to open")
    ...
    if fBoardFound:
        ...
        if user_input == 'r':
            FLS_ReadIOData(hDriver, szBoard, ulIOTimeout,0)
            print("Performing reading operation")
        else:
            print("\n*** TERMINATING Communication to Profibus MASTER ... ***")

def test_overall_status(szBoard, ulIOTimeout):
    status = {
        "driver_open": False,
        "board_found": False,
    ...
    try:
        ...
        if CIFX_NO_ERROR == wic_dll.xDriverOpen(ctypes.byref(hDriver)):
            status["driver_open"] = True
        else:
            status["error"] = "Driver not found"
            return status

        # Enumerate boards
        ulBoardIdx = c_ulong(0)
        while True:
            board_info = BoardInformation()
            result = wic_dll.xDriverEnumBoards(hDriver, ulBoardIdx, ctypes.sizeof(board_info), ctypes.byref(board_info))
            if result != CIFX_NO_ERROR:
                break

            status["board_found"] = True


            board_name = board_info.abBoardName.decode('ascii').rstrip(chr(0))
            alias = board_info.abBoardName.decode('ascii').rstrip(chr(0))
            devNR = board_info.tSystemInfo.ulDeviceNumber
            SerialNumber = board_info.tSystemInfo.ulSerialNumber
            # If no board name provided, pick the first
            if not szBoard or len(szBoard.strip()) == 0:
                print(f"No board name provided. Using detected board: {board_name}\n")
                szBoard = board_name

            status["board_name"] = szBoard
            ...
            ulBoardIdx.value += 1  # Move to next board

        # Check device connection if a board was found
        if status["board_found"] and status["board_name"]:
            if is_connection_with_device(hDriver, status["board_name"], ulIOTimeout):
                status["device_connected"] = True

    except Exception as e:
        status["error"] = f"Fatal error: {str(e)}"

    return status



def is_connection_with_device(hDriver, szBoard, ulWaitTimeout=1000) -> bool:
    ...
    if lRet != CIFX_NO_ERROR or not hDevice:
        return False

    ulState = ctypes.c_uint32(0)
    if wic_dll.xChannelHostState(hDevice, CIFX_HOST_STATE_READY, ctypes.byref(ulState), 1000) != CIFX_NO_ERROR:
        wic_dll.xChannelClose(hDevice)
        return False

    if wic_dll.xChannelBusState(hDevice, CIFX_BUS_STATE_ON, ctypes.byref(ulState), 1000) != CIFX_NO_ERROR:
        wic_dll.xChannelClose(hDevice)
        return False
    ...
    return lRet == CIFX_NO_ERROR

def test_no_driver(szBoard,ulIOTimeout):
    print("Running test: Driver detection")
    status = test_overall_status(szBoard,ulIOTimeout)

    if not status["driver_open"]:
        cifx_logger.critical("Failed to open driver,can only operate on simulation mode.")
        print("Failed: Driver is not detected")
        print("Status:", status, "\n")
        return 1
    else:
        cifx_logger.info("Passed: cifX70e driver is open, continuing workflow.")
        print("Passed: Driver is detected")
        print("Status:", status, "\n")
        return 0

# ✅ ❗️

def test_no_card(szBoard,ulIOTimeout):
    print("Running test: Driver OK, No Card Installed")
    status = test_overall_status(szBoard,ulIOTimeout)

    if status["driver_open"] and not status["board_found"]:
        cifx_logger.critical("Failed: Unexpected board found or driver missing.")
        print(" Failed: Unexpected board found or driver missing.")
        print("Status:", status, "\n")
        return 1
    else:
        cifx_logger.info("Passed: Card detected succesfully")
        print(" Passed: Card detected succesfully")
        print("Status:", status, "\n")
        return 0

def test_no_connection(szBoard,ulIOTimeout):
    print("Running test: Card Installed, Device Not Connected")
    status = test_overall_status(szBoard,ulIOTimeout)


    if status["driver_open"] and status["board_found"] and not status["device_connected"]:
        cifx_logger.critical("Failed: Device failed on connection with server")
        print("Failed: Device is not responding as expected.")
        print("Status:", status, "\n")
        return 1
    else:
        cifx_logger.info("Passed: Device connected succesfully")
        print("Passed: Device connected succesfully")
        print("Status:", status, "\n")
        return 0
...
def FLS_ReadSingleIOData(hDriver, szBoard, ulWaitTimeout):
    ...
    ulState = ctypes.c_uint32(0)
    wic_dll.xChannelHostState(hDevice, CIFX_HOST_STATE_READY, ctypes.byref(ulState), 1000)
    wic_dll.xChannelBusState(hDevice, CIFX_BUS_STATE_ON, ctypes.byref(ulState), 1000)
...
def FLS_TestWatchdog(hDriver, szBoard):
    print(f"\n--- Test Watchdog ---\r\n")
    ...
    if lRet != CIFX_NO_ERROR:
        ...
        print("Channel opened successfully, starting xChannelWatchdog command")

        if not hDevice:
            print("Invalid device handle.")
            return

        ulWatchdogValue = ctypes.c_uint32(0)
        lRet = wic_dll.xChannelWatchdog(ctypes.byref(hDevice), CIFX_WATCHDOG_START, ctypes.byref(ulWatchdogValue))
        #print(hex(ctypes.c_int(lRet)))
        unsigned_integer = lRet+(1 << 32)
        print(unsigned_integer)
        print(hex(unsigned_integer))
        print("Hdevice",hDevice, " Watch_START", CIFX_WATCHDOG_START, " ulWatchdogValue", ctypes.byref(ulWatchdogValue))
        show_error(unsigned_integer)
        #print(hex(lRet))
        ExitLoop = False
        if lRet == CIFX_NO_ERROR:
            print("\n Display WatchDogValue. Press (Q) to stop... \r\n")
            while not ExitLoop:
                #time sleep in seconds, so 10ms = 0.01s
                time.sleep(0.01)
                lRet = wic_dll.xChannelWatchdog(hDevice, CIFX_WATCHDOG_START, ctypes.byref(ulWatchdogValue))
                show_error(lRet)

               # /* Display Watchdog value */
                print(f"--> Watchdog Value = {ulWatchdogValue.value}")
                if msvcrt.kbhit():
                    ExitLoop = True

            lRet = wic_dll.xChannelWatchdog(hDevice, CIFX_WATCHDOG_STOP, ctypes.byref(ulWatchdogValue))
            show_error(lRet)
...
class PVKeyArrayAction(argparse.Action):
    def __call__(self, parser, namespace, values, option_string=None):
        data: PbBufOutWicData = getattr(namespace, 'pb_data', PbBufOutWicData())

        try:
            int_vals = [int(x.strip()) for x in values.split(',')]
        except ValueError:
            parser.error(f"Invalid integers in '{values}' for {self.dest}")
            raise argparse.ArgumentError(self, f"All values for {self.dest} must be integers")
        
        for v in int_vals:
            if not (0<= v <= 10000):
                parser.error(f"Value {v} in '{values}' for {self.dest} must be between 0 and 10000")
        
        if len(int_vals) < TRACKS_PER_VIEW_MAX:
            int_vals += [0] * (TRACKS_PER_VIEW_MAX - len(int_vals))
        elif len(int_vals) > TRACKS_PER_VIEW_MAX:
            int_vals = int_vals[:TRACKS_PER_VIEW_MAX]
        
        data.arrays[self.dest] = int_vals

        setattr(namespace, 'pb_data', data)
...
class SingleValueAction(argparse.Action):
    def __call__(self, parser, namespace, value, option_string=None):
        try:
            ...
            parser.error(f"{option_string} expects an integer.")
        if val < 0 or val > 1000:
            parser.error(f"{option_string} value must be between 0 and 10000.")

        data = getattr(namespace, 'pb_data', PbBufOutWicData())
        data.arrays[self.dest] = val
        setattr(namespace, 'pb_data', data)



def to_struct(data: dict) -> PbBufOutWic:
    struct = PbBufOutWic()

    #Timestamp now set
    now = datetime.datetime.now()
    struct.year = now.year
    struct.month = now.month
    struct.day = now.day
    struct.hours = now.hour
    struct.minutes = now.minute
    struct.seconds = now.second
    ...
    for key, value in data.arrays.items():
        if hasattr(struct, key):
            ...
            if isinstance(attr, ctypes.Array):
                array_type = type(attr)
                #array_type = ARRAY(c_uint16, TRACKS_PER_VIEW_MAX)
                clamped_vals = [max(0, min(10000, v)) for v in value]
                padded_vals = clamped_vals + [0] * (TRACKS_PER_VIEW_MAX - len(clamped_vals))
                setattr(struct, key, array_type(*padded_vals))
            else:
                int_value = max(0, min(10000, value))
                setattr(struct, key, int_value)

    return struct

def simulate_read_data():
    class SimulatedPbBufInWic:
        def __init__(self):
            self.state1 = 0
            self.state2 = 0
            self.year = 2025
            ...
            self.value_15 = 300
            self.value_16 = 400
            self.sp1 = [1000, 2000, 3000, 4000, 5000, 6000, 7000, 8000]
            self.sp2 = [9000, 10000, 11000, 12000, 13000, 14000, 15000, 16000]
            self.sp3 = [17000, 18000, 19000, 20000, 21000, 22000, 23000, 24000]
            self.sp4 = [25000, 26000, 27000, 28000, 29000, 30000, 31000, 32000]
    
    return SimulatedPbBufInWic()
